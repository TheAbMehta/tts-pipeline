...
\midrule
Machine type & \texttt{a2-highgpu-2g} \\
GPU & 2$\times$ NVIDIA A100-SXM4-40GB \\
CPU & Intel Xeon @ 2.20\,GHz (24 vCPUs) \\
RAM & 168\,GB \\
Disk & 200\,GB SSD (pd-ssd) \\
Region / Zone & \texttt{us-central1-a} \\
...
Python & 3.10.12 \\
PyTorch & 2.7.1+cu126 \\
PyTorch Lightning & 1.7.7 (patched) \\
piper-train & 1.0.0 (commit \texttt{2023.11.14-2}) \\
...
\begin{enumerate}
    \item \textbf{PyTorch version mismatch}: \texttt{piper-train} pins \texttt{torch<2}, but the GCP Deep Learning VM ships PyTorch 2.7. We installed \texttt{piper-train} with \texttt{--no-deps} and used the pre-installed PyTorch 2.7, which proved functionally compatible.

    \item \textbf{Cython extension build}: The monotonic alignment module (\texttt{monotonic\_align}) required manual Cython compilation and relocation of the resulting \texttt{.so} file to the correct package path.

    \item \textbf{LR scheduler validation}: PyTorch Lightning 1.7.7's strict scheduler API validation rejected PyTorch 2.7's \texttt{ExponentialLR}. We patched \texttt{pytorch\_lightning/core/optimizer.py} to downgrade the error to a warning.

    \item \textbf{CUDA shared libraries}: The \texttt{libcusparseLt.so.0} and \texttt{libnccl.so.2} libraries were installed but not on \texttt{LD\_LIBRARY\_PATH}. Resolved by explicitly adding NVIDIA package library paths.

    \item \textbf{DDP serialization}: Initial multi-GPU training via DDP with the \texttt{spawn} start method failed due to a tensor serialization error in PyTorch 2.7. Switching to the \texttt{ddp} strategy (fork-based) resolved this.
\end{enumerate}
...
Effective batch size & 128 (2 GPUs $\times$ 64) \\
Max training steps & 10,000 \\
Validation split & 1\% \\
Steps per epoch & $\sim$102 \\
Total epochs & $\sim$98 \\
Optimizer & AdamW \\
...
\toprule
\textbf{Step} & \textbf{Epoch} & \textbf{Elapsed (min)} & \textbf{$\mathcal{L}_G$} & \textbf{$\mathcal{L}_D$} & \textbf{$\mathcal{L}_{\text{val}}$} \\
\midrule
101  & 0  & 2.80  & 49.85 & 1.98 & 51.83 \\
203  & 1  & 4.66  & 47.29 & 1.82 & 49.11 \\
305  & 2  & 6.46  & 49.74 & 1.51 & 51.25 \\
407  & 3  & 8.18  & 51.40 & 1.61 & 53.00 \\
509  & 4  & 9.86  & 48.63 & 1.49 & 50.12 \\
611  & 5  & 11.53 & 44.50 & 2.08 & 46.58 \\
713  & 6  & 13.23 & 45.66 & 2.14 & 47.79 \\
815  & 7  & 14.90 & 49.84 & 1.89 & 51.72 \\
917  & 8  & 16.58 & 43.47 & 2.46 & 45.92 \\
1019 & 9  & 18.24 & 43.95 & 2.05 & 46.00 \\
1121 & 10 & 19.97 & 43.50 & 2.08 & 45.58 \\
1223 & 11 & 21.63 & 44.18 & 1.71 & 45.89 \\
\bottomrule
\end{tabular}
\caption{Training metrics at each validation checkpoint. $\mathcal{L}_G$ is the total generator loss, $\mathcal{L}_D$ is the discriminator loss, and $\mathcal{L}_{\text{val}}$ is the validation generator loss.}
...
Metric & Value \\
\midrule
Training throughput & 56.5 steps/min \\
Eff.\ samples/sec & $\sim$120 \\
GPU memory (GPU 0) & 26.7\,GB / 40\,GB \\
GPU memory (GPU 1) & 33.0\,GB / 40\,GB \\
GPU utilization & 93--100\% \\
Time per epoch & $\sim$1.8 min \\
Estimated total time & $\sim$3.0 hours (10,000 steps) \\
Estimated cost & $\sim$\$13.50 (spot @ \$4.50/hr) \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
    \item \textbf{ONNX Export}: \texttt{piper\_train.export\_onnx} converts the PyTorch checkpoint to ONNX format, producing \texttt{model.onnx} ($\sim$60\,MB FP32) and an accompanying \texttt{model.onnx.json} configuration file containing the phoneme ID map, inference parameters, and audio configuration.

    \item \textbf{INT8 Quantization}: Dynamic INT8 quantization via \texttt{onnxruntime.quantization.quantize\_dynamic} reduces the model to 19\,MB (30.4\% of FP32), a $3.2\times$ compression with minimal quality degradation for TTS workloads.
...
\begin{tabular}{@{}llrr@{}}
\toprule
\textbf{Target} & \textbf{Runtime} & \textbf{Latency} & \textbf{RTF} \\
\midrule
Desktop (CPU) & ONNX RT (1 thread) & 307\,ms & 0.095 \\
Desktop (CPU) & Sherpa-ONNX (4 threads) & 201\,ms & 0.074 \\
Browser & WASM (Chromium) & 1,360\,ms & 0.472 \\
\bottomrule
\end{tabular}
\caption{Measured inference latency and real-time factor for a 10-word sentence (``The quick brown fox jumps over the lazy dog.''). Desktop benchmarks on Intel Xeon @ 2.20\,GHz. Browser benchmark on Chromium via Cloudflare Pages. RTF $<$ 1.0 means faster than real-time.}
...
\begin{tabular}{@{}llrrr@{}}
\toprule
\textbf{Runtime} & \textbf{Input} & \textbf{Gen (ms)} & \textbf{Audio (s)} & \textbf{RTF} \\
\midrule
\multirow{3}{*}{ONNX RT} & Short (2 words) & 127 & 0.84 & 0.117 \\
 & Medium (10 words) & 307 & 3.03 & 0.095 \\
 & Long (30 words) & 722 & 7.86 & 0.089 \\
\midrule
\multirow{3}{*}{Sherpa-ONNX} & Short (2 words) & 67 & 0.88 & 0.069 \\
 & Medium (10 words) & 201 & 2.75 & 0.074 \\
 & Long (30 words) & 635 & 8.37 & 0.077 \\
\bottomrule
\end{tabular}
\caption{Inference latency vs.\ input length. All measurements averaged over 3 runs on CPU. RTF improves with longer inputs for ONNX Runtime (fixed overhead amortized), while Sherpa-ONNX maintains consistent RTF across lengths.}
...
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Resource} & \textbf{Duration} & \textbf{Cost (USD)} \\
\midrule
GCP A100$\times$2 spot & $\sim$3.0\,hrs & \$13.50 \\
Boot disk (200\,GB SSD) & $\sim$3.0\,hrs & \$0.05 \\
...
Training completed all 10,000 steps across 49 epochs. The final checkpoint (\texttt{epoch=49-step=10000.ckpt}, 807\,MB) was exported to ONNX (61\,MB FP32).
...
\begin{enumerate}
    \item \textbf{PyTorch 2.6 serialization}: \texttt{torch.load} now defaults to \texttt{weights\_only=True}, which rejects Piper checkpoints containing \texttt{pathlib.PosixPath}. Fix: add \texttt{PosixPath} to safe globals before loading.
    ...
    \item \textbf{Missing ONNX metadata}: Piper's \texttt{export\_onnx} does not embed the metadata fields that Sherpa-ONNX expects. Specifically, Sherpa-ONNX reads \texttt{sample\_rate}, \texttt{add\_blank}, \texttt{n\_speakers}, \texttt{voice}, and \texttt{comment} from the ONNX model's metadata properties. Without these, the model loads but inference fails at phonemization time with ``Failed to set eSpeak-ng voice.'' The fix is to patch the ONNX file after export:
